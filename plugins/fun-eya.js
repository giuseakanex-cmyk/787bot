...
handler.before = async function (m, { conn }) {
    ...
    // 🔥 LA SOLUZIONE AL LOOP: Se il messaggio è del bot stesso, ignoralo! 🔥
    ...
    if (/\beya\b/i.test(m.text)) {
        await conn.sendMessage(m.chat, { react: { text: '🫶', key: m.key } });
        await conn.reply(m.chat, "Ma Eya quella a cui vuoi un sacco di bene?Quella che aiuteresti in ogni occasione?Quella che se chiedesse un favore a te o a chiunque saresti tu il primo a porgerle la mano?", m);
    }
};
